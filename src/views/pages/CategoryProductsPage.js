          ...
          {products.length > 0 ? (
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-8 mb-8">
              {products.map((product) => {
                ...
                let originalPrice = product.metadata?.compare_price || 0;
                let discountPercentage = product.discount || 0;

                // If we have original price but no discount % calculated
                if (originalPrice > currentPrice && !discountPercentage) {
                  discountPercentage = Math.round(((originalPrice - currentPrice) / originalPrice) * 100);
                }

                // If we have discount % but no original price (reverse calculate)
                if ((!originalPrice || originalPrice <= currentPrice) && discountPercentage > 0) {
